...
CREATE    PROCEDURE [dbo].[RcStatusRC]          
 @InRegID Bigint,    
...
--RcDtime  
  ...
  IF @ERR=0  
   ...
   IF DATEDIFF(mi,@DCtime,convert(datetime,@RCDtime))>2   or @RCDtime is null  
		  BEGIN  
		   UPDATE RCDTime WITH(ROWLOCK)  SET route=1, RCKey = '',RcDtime=Null,DcTime=Getdate() WHERE RegId =@InRegID     
		   Set @OutStatus ='TRUE_'  
		   GOTO SUCCESS  
		  END  
		...
		IF @RegID<>0  
			BEGIN  
			   ...
			   ELSE  
				...
				UPDATE RCDTime WITH(ROWLOCK) SET  route=1, RCKey = '',DcTime=Getdate(),RcDtime=Null WHERE RegId =@InRegID     
			END  
